...
from pdbe_relic.conf import get_config, get_data_dir
from pdbe_relic.core.models import CompareObj, CofactorSim, Similarity
from pdbe_relic.helpers.utils import init_rdkit_templates, get_ligand_intx_chains, parse_ligand
...
class Cofactors:
    ...
    def process_entry(self) -> None:
        ...
        if cofactor_sim:
            representative_score = round(cofactor_sim.representative_sim.result.similarity_score, 3)
            self.log.info(
            ...
            ligand_cofactor_ec = pd.merge(self.cofactor_ec.loc[(self.cofactor_ec["COFACTOR_ID"]==cofactor_id)],
                                          ligand_intx_chains,
                                          left_on = "EC_NO",
                                          right_on ="ec_number").drop(columns=["EC_NO"])
            ...
            if not ligand_cofactor_ec.empty:
                ...
                cofactor_results = {
                    cofactor_sim.query_id: {
                        "template": {"id": cofactor_template_id,
                                     "similarity": round(cofactor_sim.template_sim.result.similarity_score, 3)},
                        "representative": {"id": cofactor_sim.representative_sim.target_id,
                                     "similarity": round(cofactor_sim.representative_sim.result.similarity_score, 3)},
                        "pdb_chains": ligand_cofactor_ec[["pdb_id","auth_asym_id", "struct_asym_id",
                                                          "uniprot_id","ec_number"]].to_dict('records')
                    }
                }
    ...
    def _write_cofactor_results(self, cofactor_results: dict):
        ...
        cofactor_results_path = os.path.join(self.args.out_dir, f"{ligand_id}_cofactor_annotation.json")
    ...
    def get_similarity(self, ligand: CompareObj) -> list[CofactorSim]:
        ...
        with ThreadPoolExecutor(max_workers=cpu_count() - 1) as exec:
            future_to_result = {exec.submit(template.similarity_to, 
                                            ligand, 
                                            self.cofactor_details[template.id]["threshold"]-0.01):template.id
            ...
            for future in as_completed(future_to_result):
                ...
                try:
                    ...
                    if template_sim.result.similarity_score < template_details["threshold"]:
                        continue
                    ...
                    representative_sim = self.get_representative_similarity(ligand, representative)

                    if not cofactor_sim:
                        cofactor_sim = CofactorSim(ligand.id, template_sim, representative_sim)
                    else:
                        prev_sim = (cofactor_sim.template_sim.result.similarity_score + cofactor_sim.representative_sim.result.similarity_score)/2
                        curr_sim = (template_sim.result.similarity_score + representative_sim.result.similarity_score)/2
    ...
    def get_representative(self, template_details: dict) -> CompareObj:
        ...
        representative_path = os.path.join(get_data_dir(), 
                                            get_config("cofactor", "representative_path"),
                                            f"{template_details['representative']}.cif"
                                            )
        representative_component = ccd_reader.read_pdb_cif_file(representative_path).component
        representative = CompareObj(representative_component.id, representative_component.mol_no_h)
        return representative

    def get_representative_similarity(self, query: CompareObj, representative: CompareObj) -> Similarity:
        ...
        self.log.info(f"Running similarity to representative"
                        f" {representative.id}")
